...
import { getAllUser, getUser } from "../../config/api";
...
export default function CvUser({ data }) {
  // console.log(data);
  const ref = useRef();
  const [currentData, setCurrentData] = useState(data.userData);
  const [currentTemplate, setCurrentTemplate] = useState(data.template);
